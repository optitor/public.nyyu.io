...
const CreateAvatar = () => {
    ...
    const factsDataError = useMemo(() => {
        for(let i = 0; i < factsDetail.facts.length; i++) {
            if(!factsDetail.facts[i].topic) return {index: i, item: 'topic', desc: 'Topic is required'};
            if(!factsDetail.facts[i].detail) return {index: i, item: 'detail', desc: 'Detail is required'};
        }
                            ...
                            <div className="input_div">
                                ...
                                <div className="avatar_div">
                                    ...
                                    <div className="name_div">
                                        <p>Name</p>
                                        <input className="black_input" value={avatarName} onChange={e => setAvatarName(e.target.value)}/>
                                    </div>
                                            ...
                                            return (
                                                <div key={index}>
                                                    <Select
                                                        ...
                                                        onChange={selected => {
                                                            setAvatarItems({...avatarItems, [item]: selected});
                                                        }}
                        ...
                        <>
                            <div className="input_div">                            
                                <div className="stats_div">
                                    {showError? (statsDataError.desc? <Alert severity="error">{statsDataError.desc}</Alert>: <Alert severity="success">Success! Please click Next Button</Alert>): ''}
                                    <p>Stats</p>
                                    {stats.map((stat, index) => {
                                        return (
                                            <div key={index}>
                                                <input className={`black_input ${showError && statsDataError.index === index && statsDataError.item === 'title'? 'error': ''}`} 
                                                    value={stat.title}
                                                    placeholder={`Stat${index+1}`}
                                                    onChange={e => {stats[index].title = e.target.value; setStats([...stats]);}}
                                                />
                                                <div className={`rating ${showError && statsDataError.index === index && statsDataError.item === 'stars'? 'error': ''}`}>
                                                    <Rating
                                                        name="simple-controlled"
                                                        value={stat.stars}
                                                        onChange={(event, newValue) => {
                                                            stats[index].stars = newValue; setStats([...stats]);
                                                        }}
                            ...
                            <div className="button_div">
                                ...
                                <button className="btn next" onClick={setStatsData}>Next</button>
                            </div>
                        </>
                    )}
                    {currentStep === 3 && (
                        <>
                            <div className="input_div">
                                <div className="facts_div">
                                    {showError? (factsDataError.desc? <Alert severity="error">{factsDataError.desc}</Alert>: <Alert severity="success">Success! Please click Next Button</Alert>): ''}
                                    <p>Fast facts</p>
                                    {factsDetail.facts.map((fact, index) => {
                                        return (
                                            <div key={index} className="facts">
                                                <input className={`black_input topic ${showError && factsDataError.index === index && factsDataError.item === 'topic'? 'error': ''}`} placeholder="Topic" 
                                                    value={fact.topic}
                                                    onChange={e => {factsDetail.facts[index].topic = e.target.value; setFactsDetail({...factsDetail});}}
                                                />
                                                <input className={`black_input detail ${showError && factsDataError.index === index && factsDataError.item === 'detail'? 'error': ''}`} placeholder="Detail" 
                                                    value={fact.detail}
                                                    onChange={e => {factsDetail.facts[index].detail = e.target.value; setFactsDetail({...factsDetail});}}
                                                />
                                            </div>
                                        );
                                    })}
                                    <div className="details">
                                        <p>Details</p>
                                        <textarea className={`black_input ${showError && factsDataError.item === 'details'? 'error': ''}`} name="details" rows="3" maxLength="250" 
                                            value={factsDetail.details}
                                            onChange={e => setFactsDetail({...factsDetail, details: e.target.value})}
                                        />
                            ...
                            <div className="button_div">
                                ...
                                <button className="btn next" onClick={setFactsData}>Next</button>
                            </div>
                        </>
                    )}
                    {currentStep === 4 && (
                        <>
                            <div className="input_div">
                                <div className="preview_div">
                                    <p className="name">{avatarName? avatarName: 'Gioacchino Failla'}</p>
                                    <div className="row avatarStats">
                                        <div className="col-sm-4">
                                            <img src="/avatars/failla.png" />
                                        </div>
