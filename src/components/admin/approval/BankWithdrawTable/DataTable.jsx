...
import BankWithdrawDataRow from './BankWithdrawDataRow';
...
const RoundsTable = () => {
    ...
    // Loading data
    useQuery(Query.GET_BANK_WITHDRAW_REQUESTS_BY_ADMIN, {
        onCompleted: data => {
            if(data.getBankWithdrawRequestsByAdmin) {
                bankWithdraw.fetchData(data.getBankWithdrawRequestsByAdmin);
            }
            setLoading(false);
        },
        onError: err => {
    ...
    return (
        <>
            ...
            <TableHead>
                ...
                <div className='requestedAt'>Request Time</div>
                    ...
                    {_.isEmpty(showData)?
                        ...
                        (
                            <>
                                <TableBody>
                                    {pageData.map(datum => {
                                        return <BankWithdrawDataRow key={datum.id} datum={datum} />
                                    })}
                                </TableBody>
